...
installbbr(){
    ...
    detele_kernel
    BBR_grub
    echo -e "${Tip} After rebooting the VPS, please re-run the script to turn on ${Red_font_prefix}BBR/BBR Magic Modification ${Font_color_suffix}"
    stty erase '^H' && read -p "Need to reboot the VPS before I can turn on BBR/BBR magic revision, do I reboot now ? [Y/n] :" yn
    [ -z "${yn}" ] && yn="y"
    if [[ $yn == [Yy] ]]; then
        echo -e "${Info} VPS reboot in progress..."
        reboot
    fi
}
# Installing the BBRplus kernel
installbbrplus(){
    ...
    detele_kernel
    BBR_grub
    echo -e "${Tip} After rebooting the VPS, please re-run the script to enable ${Red_font_prefix}BBRplus${Font_color_suffix}"
    stty erase '^H' && read -p "Need to reboot the VPS before turning on BBRplus, should I reboot now ? [Y/n] :" yn
    [ -z "${yn}" ] && yn="y"
    if [[ $yn == [Yy] ]]; then
        echo -e "${Info} VPS reboot in progress..."
        reboot
...
installlot(){
    ...
    detele_kernel
    BBR_grub
    echo -e "${Tip} After rebooting the VPS, please re-run the script to turn on ${Red_font_prefix}Lotserver${Font_color_suffix}"
    stty erase '^H' && read -p "Need to reboot the VPS before turning on the Lotserver, should I reboot now ? [Y/n] :" yn
    [ -z "${yn}" ] && yn="y"
    if [[ $yn == [Yy] ]]; then
        echo -e "${Info} VPS reboot in progress..."
        reboot
...
startbbrmod(){
    ...
    sysctl -p
    cd . && rm -rf bbrmod
    echo -e "${Info}Magic Modified BBR launched successfully!"
}
#Compile and enable BBR magic
...
* hard nofile 1000000">/etc/security/limits.conf
    ...
    read -p "Need to reboot the VPS before the system optimization configuration can take effect, is it reboot now ? [Y/n] :" yn
    [ -z "${yn}" ] && yn="y"
    if [[ $yn == [Yy] ]]; then
        echo -e "${Info} VPS reboot in progress..."
        reboot
...
Update_Shell(){
    ...
    if [[ ${sh_new_ver} ! = ${sh_ver} ]]; then
        ...
        if [[ ${yn} == [Yy] ]]; then
            ...
            echo -e "Script updated to latest version [ ${sh_new_ver} ] !"
        else
            echo && echo " Canceled..." && echo
        fi
    else
        echo -e "This is the latest version [ ${sh_new_ver} ] !"
        sleep 5s
    fi
}
#Start Menu
...
detele_kernel(){
    if [[ "${release}" == "centos" ]]; then
        ...
        if [ "${rpm_total}" > "1" ]; then
            echo -e "${rpm_total} remaining kernels detected, starting uninstallation..."
            for((integer = 1; integer <= ${rpm_total}; integer++)); do
                rpm_del=`rpm -qa | grep kernel | grep -v "${kernel_version}" | grep -v "noarch" | head -${integer}`
                echo -e "Starting to uninstall ${rpm_del} kernel..."
                rpm --nodeps -e ${rpm_del}
                echo -e "Uninstalling ${rpm_del} kernel is complete, continue..."
            done
            echo --nodeps -e "Kernel uninstallation complete, continue..."
        else
        ...
        if [ "${deb_total}" > "1" ]; then
            echo -e "${deb_total} remaining kernels detected, start uninstalling..."
            for((integer = 1; integer <= ${deb_total}; integer++)); do
                deb_del=`dpkg -l|grep linux-image | awk '{print $2}' | grep -v "${kernel_version}" | head -${integer}`
                echo -e "Starting to uninstall ${deb_del} kernel..."
                apt-get purge -y ${deb_del}
                echo -e "Uninstalling ${deb_del} kernel is complete, continue..."
            done
            echo -e "Kernel uninstallation complete, continue..."
        else
            echo -e " Incorrect number of cores detected, please check !" && exit 1
        fi
    fi
...
check_sys_bbr(){
    ...
    if [[ "${release}" == "centos" ]]; then
        if [[ ${version} -ge "6" ]]; then
            ...
            echo -e "${Error} BBR kernel not supported on current system ${release} ${version} ${bit} !" && exit 1
        fi
    elif [[ "${release}" == "debian" ]]; then
...
check_sys_bbrplus(){
    ...
    if [[ "${release}" == "centos" ]]; then
        if [[ ${version} -ge "6" ]]; then
            ...
            echo -e "${Error} BBRplus kernel not supported on current system ${release} ${version} ${bit} !" && exit 1
        fi
    elif [[ "${release}" == "debian" ]]; then
        if [[ ${version} -ge "8" ]]; then
            installbbrplus
...
check_status(){
    ...
    elif [[ ${kernel_version} = "3.10.0" || ${kernel_version} = "3.16.0" || ${kernel_version} = "3.2.0" || ${kernel_version} = "4.8.0" || ${kernel_version} = "3.13.0" || ${kernel_version} = "2.6.32" || ${kernel_version} = "4.9.0" ]] version} = "3.13.0" || ${kernel_version} = "2.6.32" || ${kernel_version} = "4.9.0" ]]; then
    ...
    if [[ ${kernel_status} == "Lotserver" ]]; then
        ...
        if [[ ${run_status} == "bbr" ]]; then
            ...
            if [[ ${run_status} == "tcp_bbr" ]]; then
                ...
                run_status="BBR startup failure"
            fi
        elif [[ ${run_status} == "tsunami" ]]; then
            run_status=`lsmod | grep "tsunami" | awk '{print $1}'`
            if [[ ${run_status} == "tcp_tsunami" ]]; then
                run_status="BBR Magic Revision launched successfully"
            else 
            ...
            if [[ ${run_status} == "tcp_nanqinlang" ]]; then
                ...
                run_status="Violent BBR Magic Mod failed to start"
            fi
        else 
            run_status="Acceleration module not installed"
        fi
        ...
        if [[ ${run_status} == "bbrplus" ]]; then
            ...
            if [[ ${run_status} == "tcp_bbrplus" ]]; then
                ...
                run_status="BBRplus startup failed"
            fi
        else 
            run_status="Acceleration module not installed"
        fi
